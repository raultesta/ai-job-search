...
// return before any network call is made — safe to run freely. The one real
// live-network tests live in live-smoke.test.ts, kept deliberately minimal.
...
describe("Himalayas CLI flag validation", () => {
  ...
  describe("help", () => {
    test("no args prints help and exits 1", async () => {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stdout).toMatch(/himalayas-cli/);
    });
