...
// touching Remotive's rate-limited API. The one real live-network test
// lives in live-smoke.test.ts, kept deliberately minimal.
...
describe("Remotive CLI flag validation", () => {
  ...
  describe("help", () => {
    test("no args prints help and exits 1", async () => {
      ...
      expect(result.exitCode).toBe(1);
      expect(result.stdout).toMatch(/remotive-cli/);
    });
